...
static void compress_ve_float_3_vl(zfp_stream* stream, const zfp_field* field, int64_t bmin, int64_t bmax)
{
...
void compress_ve_float_3(zfp_stream* stream, const zfp_field* field)
{
  ...
  #pragma omp parallel private(i,nt,it,bmin,bmax)
  {
    ...
    for (i = bmin; i < bmax; i += VL) {
      ...
      compress_ve_float_3_vl(stream, field, i, i + n);
    }
  }
#ifdef _FTRACE
