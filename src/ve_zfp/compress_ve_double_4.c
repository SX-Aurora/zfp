...
static void compress_ve_double_4_vl(zfp_stream* stream, const zfp_field* field, uint64_t bmin, uint64_t bmax)
{
  const int vl = (bmax - bmin);
  double fblock_in[vl * BLOCK_SIZE];
...
void compress_ve_double_4(zfp_stream* stream, const zfp_field* field)
{
  ...
  #pragma omp parallel private(i,nt,it,bmin,bmax)
  {
    ...
    for (i = bmin; i < bmax; i += VL) {
      ...
      compress_ve_double_4_vl(stream, field, i, i + n);
    }
  }
#ifdef _FTRACE
