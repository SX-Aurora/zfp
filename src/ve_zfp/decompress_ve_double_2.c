...
static void decompress_ve_double_2_vl(zfp_stream* stream, zfp_field* field, uint64_t bmin, uint64_t bmax)
{
...
void decompress_ve_double_2(zfp_stream* stream, zfp_field* field)
{
  ...
  #pragma omp parallel private(i,nt,it,bmin,bmax)
  {
    ...
    for (i = bmin; i < bmax; i += VL) {
      ...
      decompress_ve_double_2_vl(stream, field, i, i + n);
    }
  }
#ifdef _FTRACE
