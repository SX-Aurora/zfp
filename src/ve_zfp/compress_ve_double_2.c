...
#define LOG2      log2
#define POW       pow
#define FWD_XFORM fwd_xform_2
#define PERM      perm_2

#include "ve_bitstream.c"
#include "fwd_xform_2.c"
#include "gather_vl_2.c"
#include "encode_block_vl.c"

static void compress_ve_double_2_vl(zfp_stream* stream, const zfp_field* field, int64_t bmin, int64_t bmax)
{
  ...
  gather_vl_2(field, fblock_in, bmin, bmax);

  encode_block_vl(stream, &lstream, fblock_in, bmin, bmax);
}

void compress_ve_double_2(zfp_stream* stream, const zfp_field* field)
{
  int64_t i, nx, ny, bx, by, blocks, bmin, bmax;
  int it, nt;
  ...
  #pragma omp parallel private(i,nt,it,bmin,bmax)
  {
    ...
    for (i = bmin; i < bmax; i += VL) {
      ...
      compress_ve_double_2_vl(stream, field, i, i + n);
    }
  }
#ifdef _FTRACE
