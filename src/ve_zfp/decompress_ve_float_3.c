...
static void decompress_ve_float_3_vl(zfp_stream* stream, zfp_field* field, uint64_t bmin, uint64_t bmax)
{
...
void decompress_ve_float_3(zfp_stream* stream, zfp_field* field)
{
  ...
  #pragma omp parallel private(i,nt,it,bmin,bmax)
  {
    ...
    for (i = bmin; i < bmax; i += VL) {
      ...
      decompress_ve_float_3_vl(stream, field, i, i + n);
    }
  }
#ifdef _FTRACE
