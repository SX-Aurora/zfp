static void scatter_vl_4(const zfp_field* field, Scalar *fblock, uint64_t bmin, uint64_t bmax)
{
  ...
  const uint64_t sw = field->sw ? field->sw : nx * ny * nz;
  uint64_t blk_idx;
  ...
  if (nx % 4 != 0 || ny % 4 != 0 || nz % 4 != 0 || nw % 4 != 0) {
    ...
    for (blk_idx = bmin; blk_idx < bmax; blk_idx++) {
      ...
      b /= bz;
      w = 4 * b;
      if (x + 4 > nx || y + 4 > ny || z + 4 > nz || w + 4 > nw) partial_scatter++;
    }
  }
  ...
  if (!partial_scatter) {
    ...
    for (blk_idx = bmin; blk_idx < bmax; blk_idx++) {
      ...
      b /= bz;
      w = 4 * b;
      Scalar *p = &data[x * sx + y * sy + z * sz + w * sw]; 
        ...
        for (iz = 0; iz < 4; iz++, p += sz - 4 * sy) {
          ...
          for (iy = 0; iy < 4; iy++, p += sy - 4 * sx) {
            ...
            for (ix = 0; ix < 4; ix++, p += sx) {
              *p = fblock[(64 * iw + 16 * iz + 4 * iy + ix) * vl + (blk_idx - bmin)];
            }
          }
        }
      }
      ...
      for (iw = 0; iw < 4; iw++) {
        ...
        for (iz = 0; iz < 4; iz++) {
          ...
          for (iy = 0; iy < 4; iy++) {
            #pragma _NEC unroll_completely
            for (ix = 0; ix < 4; ix++) {
              if (x+ix < nx && y+iy < ny && z+iz < nz && w+iw < nw) {
                data[(x+ix) * sx + (y+iy) * sy + (z+iz) * sz + (w+iw) * sw] = fblock[(64 * iw + 16 * iz + 4 * iy + ix) * vl + (blk_idx - bmin)];
              }
            }
          }
        }
