...
static void decompress_ve_double_4_vl(zfp_stream* stream, const zfp_field* field, uint64_t bmin, uint64_t bmax)
{
...
void decompress_ve_double_4(zfp_stream* stream, const zfp_field* field)
{
  ...
  #pragma omp parallel private(i,nt,it,bmin,bmax)
  {
    ...
    for (blk_idx = bmin; blk_idx < bmax; blk_idx += VL) {
      ...
      decompress_ve_double_4_vl(stream, field, blk_idx, blk_idx + n);
    }
  }
